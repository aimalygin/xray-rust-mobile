...
The prepare workflow artifact is retained for 30 days. The tag workflow does
not trust or republish that cached file: it rebuilds the iOS-only archive from
the tag and requires the same locked checksum.
...
5. Push the metadata changes to `main`, then dispatch **Prepare release** for
   the exact version. The workflow builds the canonical iOS ZIP and opens a PR
   that updates both `Package.swift` and `release/artifacts.env`.
...
- independently rebuilds the iOS-only SwiftPM ZIP from the tag and requires
  its checksum to match the prepared lock;
